...
class SparkLLMClient:
    ...
    def gen_params(self, messages: list, user_id: str, model_kwargs: Optional[dict] = None) -> dict:
        data = {
            "header": {"app_id": self.app_id, "uid": user_id[:32] if user_id else None},
        ...
        while True:
            ...
            if "error" in content:
                if content["status_code"] == 401:
                    raise SparkError(
                        "[Spark] The credentials you provided are incorrect. Please double-check and fill them in again."
                    )
                elif content["status_code"] == 403:
                    raise SparkError(
                        "[Spark] Sorry, the credentials you provided are access denied. Please try again after obtaining the necessary permissions."
                    )
                else:
